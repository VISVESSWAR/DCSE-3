...
router.post('/:facultyId', restrictTo('faculty'), async (req, res) => {
  try {
    ...
    const { year, period, facultyAcknowledgement, facultyFinalSignature } = req.body;
    
    // Find existing report or create new one
    let report = await CRReport.findOne({ 'faculty.facultyId': facultyId, year });
    if (!report) {
      // Get faculty details
...
router.post('/:reportId/self-assessment', restrictTo('faculty'), updateSelfAssessment);

// Upload attachments for self-assessment (faculty only)
router.post('/:reportId/self-assessment/attachments', restrictTo('faculty'), upload.array('attachments'), async (req, res) => {
  try {
    const CRReport = require('../models/CRReport');
    const report = await CRReport.findById(req.params.reportId);
