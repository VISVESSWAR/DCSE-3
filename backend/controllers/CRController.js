...
const getOrCreateCRReport = async (req, res) => {
  try {
    ...
    let report = await CRReport.findOne({ 'faculty.facultyId': facultyId, year: req.query.year });
    if (!report) {
...
const updateHODSection = async (req, res) => {
    try {
      ...
      report.hodSection = { ...update, hodSignature: req.user.name, hodDate: new Date() };
...
const finalizeReport = async (req, res) => {
  try {
    const { reportId } = req.params;
    const report = await CRReport.findByIdAndUpdate(reportId, { status: 'finalized' }, { new: true });
    res.json(report);
  } catch (err) {
    res.status(400).json({ message: err.message });
  }
...
const downloadReport = async (req, res) => {
  try {
    ...
    const hodSectionHtml = `
      ...
      <div>6. Comments on class records: ${hod.performance?.classRecords || ''}</div>
      <div>7. Contributions: ${hod.performance?.contributions || ''}</div>
      <div>8. Research ability: ${hod.performance?.researchAbility || ''}</div>
      <div>9. Professional standing: ${hod.performance?.professionalStanding || ''}</div>
    ...
    const potentialSectionHtml = `
      ...
      <div>G. Academic leadership: ${hod.potential?.academicLeadership || ''}</div>
    ...
    if (self.subjectsTaught && self.subjectsTaught.length > 0) {
      selfAssessmentHtml += '<table class="table"><tr><th>Subject</th><th>Contact Hours</th><th>Appeared</th><th>Passed</th><th>Remarks</th></tr>';
      self.subjectsTaught.forEach(s => {
        selfAssessmentHtml += `<tr><td>${s.subject || ''}</td><td>${s.contactHours || ''}</td><td>${s.studentsAppeared || ''}</td><td>${s.studentsPassed || ''}</td><td>${s.remarks || ''}</td></tr>`;
      });
      selfAssessmentHtml += '</table>';
    }
    if (self.examResults) selfAssessmentHtml += `<div>Exam Results: ${self.examResults}</div>`;
    if (self.contributions) selfAssessmentHtml += `<div>Contributions: ${self.contributions}</div>`;
    ...
    if (report.attachments && report.attachments.length > 0) {
      ...
      report.attachments.forEach(a => {
        attachmentsHtml += `<li><a href="${process.env.BASE_URL || ''}/uploads/${a.filename}">${a.filename}</a></li>`;
      });
      attachmentsHtml += '</ul>';
    }
    ...
    templateHtml = templateHtml
      ...
      .replace(/{{HOD_DATE}}/g, hod.hodDate ? new Date(hod.hodDate).toLocaleDateString() : '')
...
const downloadCRPDF = async (req, res) => {
  try {
    ...
    const report = await CRReport.findById(reportId);
    if (!report) return res.status(404).json({ message: 'CR Report not found' });
