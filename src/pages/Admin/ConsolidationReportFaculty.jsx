...
export default function ConsolidationReportFaculty() {
  ...
  const facultyByPosition = useMemo(
    () => groupBy(faculty, (f) => f.position || "Unknown"),
    [faculty]
  );
    ...
    async function fetchData() {
      ...
      try {
        const [facRes, schRes, odRes, pubRes] = await Promise.all([
          axios.get(`${backendUrl}/api/faculty`, {
            headers: { "x-user-email": user.email },
          }),
          axios.get(`${backendUrl}/api/pgscholars`, {
            headers: { "x-user-email": user.email },
          }),
          axios.get(`${backendUrl}/api/odrequests`, {
            headers: { "x-user-email": user.email },
          }),
  ...
  useEffect(() => {
    async function fetchCRReports() {
      setLoadingCR(true);
      try {
        const res = await axios.get(`${backendUrl}/api/crreport`, {
          headers: { "x-user-email": user.email },
        });
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <ul className="text-base space-y-1">
                ...
                <li>
                  <b>Position:</b> {selectedFaculty.position || "-"}
                </li>
                <li>
      ...
      <section className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          ...
          <div className="bg-white rounded-xl shadow p-5 flex flex-col items-center">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart
                data={Object.entries(facultyByPosition).map(([pos, count]) => ({
                  pos,
                  count,
                }))}
                layout="vertical"
