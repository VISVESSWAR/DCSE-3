...
export default function ConsolidationReport() {
  ...
  function PieChart({ data, colors, label }) {
    ...
    const slices = entries.map(([k, v], i) => {
      ...
      return <path key={k} d={d} fill={colors[i % colors.length]} />;
    });
    return (
      <div className="flex flex-col items-center">
        <svg width="120" height="120" viewBox="0 0 100 100">
        ...
        <div className="flex flex-wrap gap-2 mt-2 text-xs justify-center">
          {entries.map(([k, v], i) => (
            <div key={k} className="flex items-center gap-1"><span className="w-3 h-3 rounded-full inline-block" style={{background: colors[i % colors.length]}}></span>{k} ({v})</div>
          ))}
        </div>
